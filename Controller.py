...
from DataVis import XY_PlotLabeled
...
class Controls():
				...
				if population != False:
					...
					if self.getRawUserInput('Type "y" to accept query parameters.').lower() == 'y':
						...
						if subcategoryLabel == "None":
							yAxisLabel = category.replace('_', ' ')
							if populationClockNumber == '0000':
								# Entire category with all employees
								queryOutput = self.catDB.queryEntireCategoryAllEmployees(subcategory)
								
							else:
								# Entire category with single employee
								queryOutput = self.catDB.queryEntireCategorySingleEmployee(subcategory, populationClockNumber)
						
						else:
							yAxisLabel = category.replace('_', ' ')
							yAxisLabel += ": %s" % subcategoryLabel.replace('_', ' ')
							if populationClockNumber == '0000':
								# Specific sub-category with all employees
								queryOutput = self.catDB.querySubcategoryAllEmployees(category, subcategory)
								
							else:
								# Specific sub-category with single employee
								queryOutput = self.catDB.querySubcategorySingleEmployee(category, subcategory, populationClockNumber)
						
						plotTitle = "Hours Worked Per Month for:\nEmployee - %s\n%s" % (population, yAxisLabel)
						xValueLabels, yValues = self.convertQueryOutputToXLablesYvalues(queryOutput)
						xAxisLabel = "Month"
						self.DataVis.createXYLabeledPlot(xValueLabels, xAxisLabel ,yValues, yAxisLabel, plotTitle)
	...
	def convertQueryOutputToXLablesYvalues(self, QueryOutput):
		xValueLabels = []
		yValues = []
		for item in QueryOutput:
			xValueLabels.append(str("%s" % item[0]))
			yValues.append(item[1])
		return xValueLabels, yValues 
